...
#
# Note: These are different from the workflow script environment variables.
# The workflow script uses METADATA_EXTRACTION_SCRIPT to point to this wrapper script.

# Download script path
DOWNLOAD_SCRIPT = os.getenv(
    'PYPRESERVICA_DOWNLOAD_SCRIPT',
    "pypreservica scripts/download_preservica_assets.py"
)
...
def main():
    ...
    # Ensure csv_output is a Path object and has proper extension
    if not str(csv_output).endswith('.csv'):
