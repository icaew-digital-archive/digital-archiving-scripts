class ICAEWBehaviors {
  ...
  async *run(ctx) {
    ...
    const { sleep, scrollAndClick, isInViewport } = ctx.Lib;

    // ctx.Lib.waitUntilNode expects an XPath string + root Node
    // (it calls document.evaluate(xpath, root, ...) internally), not a
    // predicate function + timeout. This script uses CSS selectors, so we
    // poll for those directly instead of misusing waitUntilNode.
    const waitForSelector = async (selector, timeout, interval = 200) => {
    ...
    try {
      const cookieButton = await waitForSelector(
    ...
    try {
      const bannerCookie = await waitForSelector("#banner-cookie > span", 3000);
      if (bannerCookie) {
    ...
    try {
      const surveyToggle = await waitForSelector("#hj-survey-toggle-1", 3000);
      if (surveyToggle) {
    ...
    try {
      ...
      for (const selector of filterSelectors) {
        try {
          parentElement = await waitForSelector(selector, 5000);
          if (parentElement) {
