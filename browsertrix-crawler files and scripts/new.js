class ICAEWBehaviors {
    ...
    async *run(ctx) {
      ...
      try {
        ...
        if (cookieButton) {
          const style = getComputedStyle(cookieButton);
          if (style.display !== "none" && style.visibility !== "hidden") {
            await scrollAndClick(cookieButton);
            ctx.log("Clicked cookie consent button");
      ...
      try {
        ...
        if (bannerCookie) {
          const style = getComputedStyle(bannerCookie);
          if (style.display !== "none" && style.visibility !== "hidden") {
            await scrollAndClick(bannerCookie);
            ctx.log("Clicked banner cookie");
      ...
      try {
        ...
        if (surveyToggle) {
          const style = getComputedStyle(surveyToggle);
          if (style.display !== "none" && style.visibility !== "hidden") {
            await scrollAndClick(surveyToggle);
            ctx.log("Clicked survey toggle");
      ...
      try {
        ...
        for (const selector of filterSelectors) {
          try {
            ...
            if (parentElement) {
              ctx.log(`Found filter using selector: ${selector}`);
              break;
            }
          } catch (e) {
            continue;
      ...
      while (iteration < maxIterations) {
        ...
        for (let i = 0; i < uniqueNextButtons.length; i++) {
          ...
          if (style.display !== "none" && style.visibility !== "hidden" && 
            ...
            clickedAny = true;
            ctx.log(`Clicked "Next" button (${i + 1}/${uniqueNextButtons.length}, iteration ${iteration + 1})`);
            yield ctx.Lib.getState(ctx, `Clicked "Next" button (iteration ${iteration + 1})`);
      ...
      while (moreLinkIterations < maxMoreLinkIterations) {
        ...
        if (element) {
          ...
          if (style.display !== "none" && style.visibility !== "hidden") {
            ...
            ctx.log(`Clicked more-link (iteration ${moreLinkIterations + 1})`);
